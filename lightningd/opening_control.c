				    ...
				    bool supports_shutdown)
{
	...
	json_add_num(response, "open_channel_version", fc->is_v2 ? 2 : 1);
					 ...
					 struct uncommitted_channel *uc)
{
	...
	if (!add_remote_witnesses(utx, remote_witnesses)) {
		...
		uncommitted_channel_disconnect(uc, "internal error in updating tx witnesses");
		goto cleanup;
	}
...

{
	...
	in = set->inputs[set->last_index];
	bitcoind_gettxout(bitcoind, &in->prevtx_txid,
			  in->prevtx_vout,
			 ...
			 struct uncommitted_channel *uc)
{
	...
	set->last_index = 0;
	bitcoind_gettxout(openingd->ld->topology->bitcoind,
...
#if EXPERIMENTAL_FEATURES
	fc->is_v2 = feature_offered(peer->features, OPT_FUNDCHANNEL_V2);
#else
	fc->is_v2 = false;
#endif
