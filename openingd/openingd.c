					  ...
					  struct bitcoin_tx **tx)
{
	...
	state->channel = new_initial_channel(state,
					     ...
					     &state->our_funding_pubkey,
					     &state->their_funding_pubkey,
					     !amount_sat_eq(state->funding, AMOUNT_SAT(0)),
					     /* Funder is local */
					     LOCAL);
...
{
	struct amount_sat remote_funding, local_funding;
	...
	if (!state->df->our_outputs)
		status_failed(STATUS_FAIL_MASTER_IO,
			      "Error deriving change key %u", change_keyindex);

	// FIXME: double check this reserve logic ???
	/* This reserves 1% of our channel contrib (rounded up) */
	set_reserve(state);

	if (!check_reserves(state))
		return NULL;

	msg = towire_funding_compose(tmpctx, &state->channel_id,
				     state->localconf.channel_reserve,
				     (const struct input_info **)state->df->our_inputs,
				     (const struct output_info **)state->df->our_outputs);

	sync_crypto_write(state->pps, take(msg));

	peer_billboard(false,
		       "Opening channel: funding_compose sent, "
		       "now waiting for their funding_compose");
	...
	peer_billboard(false,
		       "Opening channel: funding_compose rcvd, composing commitment_signed");
	...
	if (!check_config_bounds_reserves_required(state, &state->remoteconf, true))
		return NULL;

	// TODO: send to master to check that their inputs are actually 'good'

	/* Move our push_msat over to the other peer */
	if (!amount_sat_sub_msat(&local_funding_msat, state->funding, state->push_msat))
		peer_failed(state->pps,
			    &state->channel_id,
	...
	msg = towire_opening_dual_funding_signed(tmpctx,
						 state->pps,
						 local_commit,
						 &their_sig,
						 NULL,
	...
	peer_billboard(false,
		       "Opening channel: Got sigs from master. Sending funding_signed2");
	...
	/* This is a loop which handles gossip until we get a non-gossip msg */
	// TODO: they may never send us a funding_signed2
	msg = opening_negotiate_msg(tmpctx, state, false);
	if (!msg)
		return NULL;
	...
	// TODO: peer fails here but they may succeed at establishing the channel
		peer_failed(state->pps,
			    &state->channel_id,
...
static u8 *funder_channel_complete(struct state *state)
{
	...
	return towire_opening_funder_reply(tmpctx,
					   &state->remoteconf,
					   tx,
					   &sig,
					   state->pps,
...
static u8 *funder_channel(struct state *state,
			  ...
			  struct input_info **our_inputs TAKES,
			  const struct ext_key *bip32_base)
{
	struct channel_id id_in;
	u8 *msg;
	...
	if (state->df) {
		/* FIXME: add bolt reference *
		 *   - MUST set the `contrib_count` to the total of
		 *   `num_inputs` plus `num_outputs' that will be included
		 *   in `funding_compose`, with minimum 2. */
		state->df->contrib_count = tal_count(utxos) + !amount_sat_eq(change, AMOUNT_SAT(0));
	...
	if (!state->df) {
		...
		peer_failed(state->pps,
			    &state->channel_id,
			    "Bad state: started v2 channel open but can't parse response",
			    tal_hex(msg, msg));
#endif
	}
	...
	if (!state->df) {
		...
		if (!check_config_bounds_no_reserve(state, &state->remoteconf, true))
			goto fail;
	}
	...
	if (!state->df) {
		...
		msg = funder_finalize_channel_setup2(state, utxos,
						     our_inputs,
						     change, change_keyindex,
						     bip32_base);
		if (!msg)
			goto fail;
...
static void check_next_input(struct bitcoind *bitcoind,
		       ...
		       void *arg)
{
	struct input_check *check = arg;
	size_t index = check->index;
	struct state *state = check->state;

	if (memcmp(txout->script, check->inputs[index]->prevtx_scriptpubkey,
		   tal_bytelen(txout->script)) != 0)
		peer_failed(state->pps,
			    &state->channel_id,
			    "Verification of input script %ld failed",
			    index);

	if (!amount_sat_add(&check->total_in, check->total_in, txout->amount))
		fatal("Overflow in remote input_tx addition %s + %s",
		      type_to_string(tmpctx, struct amount_sat,
				     &check->total_in),
		      type_to_string(tmpctx, struct amount_sat,
				     &txout->amount));

	if (index < tal_count(check->inputs) - 1) {
		check->index = index++;

		bitcoind_gettxout(bitcoind, remote_inputs[index]->prev_txid,
				  remote_inputs[index]->prev_vout, after_call,
				  check_arg);
	} else
		// FIXME: divide this up
		continue_channel_establishment();
}

static void verify_input_scripts(struct state *state,
				 struct input_info **inputs)
{
	struct input_check *check_arg = tal(inputs, struct input_check);

	check_arg->index = 0;
	check_arg->inputs = inputs;
	check_arg->state = state;
	check_arg->total_in = AMOUNT_SAT(0);

	// Verify that this input says who it says it is via bitcoind
	bitcoind_gettxout(bitcoind, remote_inputs[0]->prev_txid,
			  remote_inputs[0]->prev_vout, check_next_input,
			  check_arg);
}
*/
			   ...
			   const u8 *msg)
{
	 ...
	 *     DER-encoded compressed secp256k1 pubkeys.
	 */
	if (!fromwire_open_channel2(msg, &state->df->chain_hash,
				    &state->channel_id,
				    &state->funding,
				    &state->push_msat,
				    &state->remoteconf.dust_limit,
				    ...
				    struct amount_sat our_funding)
{
	...
	return towire_opening_dual_accepted(tmpctx,
					    &state->their_funding_pubkey,
					    (const struct input_info **)state->df->their_inputs,
					    (const struct output_info **)state->df->their_outputs);
}
...
static u8 *continue_dual_fund_request(struct state *state)
{
	...
	if (!amount_msat_add_sat(&local_msat, state->push_msat,
				 state->df->accepter_funding))
		peer_failed(state->pps,
			    &state->channel_id,
			    "Overflow adding our funding %s with push_msat %s",
			    type_to_string(tmpctx, struct amount_sat,
				    	   &state->df->accepter_funding),
			    type_to_string(tmpctx, struct amount_msat,
				    	   &state->push_msat));
	...
	peer_billboard(false,
		       "Incoming channel: commitment_signed sent, now waiting on funding_signed2");
